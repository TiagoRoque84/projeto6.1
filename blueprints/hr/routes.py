...
def employee_docs(emp_id):
    ...
    if form.validate_on_submit():
        ...
        d = EmployeeDocument(employee_id=emp.id, tipo=form.tipo.data, descricao=form.descricao.data, arquivo_path=path)
    ...
    if q:
        docs = [d for d in docs if (d.tipo and q in d.tipo.lower()) or (d.descricao and q in d.descricao.lower())]
...
def employee_docs_delete(doc_id):
    ...
    try:
        ...
        if doc.arquivo_path:
            full_path = os.path.join(current_app.root_path, current_app.config['UPLOAD_FOLDER'], doc.arquivo_path)
    ...
    return redirect(url_for('rh.employee_docs', emp_id=employee_id))
# --- FIM: NOVA ROTA ---
...
def api_cep(cep):
    ...
    except Exception: return {"erro": True}, 400
