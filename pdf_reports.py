...
# ... (outras funções de PDF como employee_pdf, company_pdf, etc., continuam aqui) ...
...
def employee_pdf(buffer, app, e):
    ...
    if getattr(e, 'foto_path', None):
        try:
            ...
            if photo_abs and os.path.exists(photo_abs): photo_flow = Image(photo_abs, width=3.0*cm, height=4.0*cm)
        except Exception: photo_flow = None
    header = Table([[title, photo_flow]], colWidths=[13.0*cm, 4.0*cm])
    header.setStyle(TableStyle([('VALIGN',(0,0),(-1,-1),'TOP')]))
    elems.extend([header, Spacer(1, 0.3*cm)])
    data = [ ["Nome", P(e.nome), "Empresa", P(e.company.razao_social if e.company else "")], ["Função", P(e.funcao.nome if e.funcao else ""), "Ativo", P("Sim" if e.ativo else "Não")], ["CPF", P(e.cpf), "RG", P(e.rg)], ["Nascimento", P(format_date(e.data_nascimento)), "Gênero", P(e.genero)], ["Estado civil", P(e.estado_civil), "Admissão", P(format_date(e.data_admissao))], ["Salário (R$)", P(e.salario), "Jornada", P(e.jornada)], ["Telefone", P(e.fone), "Celular", P(e.celular)], ["E-mail", P(e.email), "", ""], ["Filho < 14", P("Sim" if getattr(e,'filho_menor14',None) else ("Não" if getattr(e,'filho_menor14',None) is not None else "")), "Escolaridade", P(getattr(e,'escolaridade', ""))], ["Endereço", P(f"{_s(e.logradouro)}, {_s(e.numero)} {_s(e.complemento)}"), "CEP", P(e.cep)], ["Bairro/Cidade/UF", P(f"{_s(e.bairro)} / {_s(e.cidade)} / {_s(e.uf)}"), "", ""], ["Banco", P(e.banco), "Agência/Conta", P(f"{_s(e.agencia)} / {_s(e.conta)}")], ["Tipo de conta", P(e.tipo_conta), "PIX", P(f"{_s(e.pix_tipo)}: {_s(e.pix_chave)}")], ["ASO", P(e.aso_tipo), "Validade", P(format_date(e.aso_validade))], ["CNH", P(e.cnh), "CNH Validade", P(format_date(e.cnh_validade))], ["Toxicológico", "", "Validade", P(format_date(e.exame_toxico_validade))], ]
...
def company_pdf(buffer, app, c):
    ...
    data = [ ["Razão Social", P(c.razao_social)], ["Nome Fantasia", P(c.nome_fantasia)], ["CNPJ", P(c.cnpj)], ["Inscrição Estadual", P(c.inscricao_estadual)], ["Endereço", P(f"{_s(c.logradouro)}, {_s(c.numero)} {_s(c.complemento)}")], ["Bairro/Cidade/UF", P(f"{_s(c.bairro)} / {_s(c.cidade)} / {_s(c.uf)}")], ["CEP", P(c.cep)], ["Status", P("Ativa" if getattr(c,'ativa',True) else "Inativa")], ["Emails alerta", P(getattr(c,'alert_email',""))], ["WhatsApp alerta", P(getattr(c,'alert_whatsapp',""))], ]
...
def epi_saida_pdf(buffer, app, mov):
    ...
    assinatura_data=[["_________________________________________"],[P(f"<b>{_s(mov.retirado_por)}</b>")],[P(f"CPF: {_s(mov.employee.cpf if mov.employee else'Não informado')}")],]
    assinatura_table=Table(assinatura_data,colWidths=[17*cm])
    assinatura_table.setStyle(TableStyle([('ALIGN',(0,0),(-1,-1),'CENTER'),('FONTSIZE',(0,0),(-1,-1),10)]))
    elems.append(assinatura_table)
    elems.append(Spacer(1,2*cm))
    elems.append(Paragraph(f"Gerado em {format_date(_date.today())}",BodyRight))
    doc.build(elems)

# --- FUNÇÃO DO RELATÓRIO DE CAIXA ATUALIZADA ---
def pdv_summary_pdf(buffer, app, movements, start_date, end_date):
    ...
    # Dicionários para armazenar os totais
    ...
    for mov in movements:
        ...
        # Acumula totais por tipo e forma de pagamento
        if mov.tipo in ['VENDA', 'PAGAMENTO']:
            totals[mov.pagamento] += valor
    ...
    table.setStyle(TableStyle([
        ...
        ('ALIGN', (0,0), (-1,-1), 'LEFT'), ('ALIGN', (4,1), (4,-1), 'RIGHT'),
    ]))
    elems.append(table)
    elems.append(Spacer(1, 1*cm))
    ...
    summary_data.append(['', '']) # Linha em branco para separar
